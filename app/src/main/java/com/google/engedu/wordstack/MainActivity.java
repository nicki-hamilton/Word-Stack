...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onStartGame(View view) {
        ...
        int wordOne = 0, wordTwo = 0, mergedlist = 0;

        while(wordOne < word1.length() && wordTwo < word2.length())
        {
            if(random.nextInt(2) == 0)
            {
                merged[mergedlist++] = word1.charAt(wordOne++);
            }
            else
            {
                merged[mergedlist++] = word2.charAt(wordTwo++);
            }
        }
        while(wordOne < word1.length())
        {
            merged[mergedlist++] = word1.charAt(wordOne++);
        }
        while(wordTwo < word2.length())
        {
            merged[mergedlist++] = word2.charAt(wordTwo++);
        }
